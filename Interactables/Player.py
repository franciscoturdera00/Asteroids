...
class Player:
    ...
    def draw(self, screen: pygame.Surface):
        ...
        for point in self.player_shape:
            rotated_x, rotated_y = self.calculate_new_position(point)
        ...
        if self.boosting and random.random() < self.BOOST_SHOW_PERCENTAGE:
            ...
            for point in self.boost_shape:
                rotated_boost_x, rotated_boost_y = self.calculate_new_position(point)
    ...
    def _accelerate(self, speed=ACCELERATION):
        ...
        self.velocity[0] = self.velocity[0] + math.cos(self._angle_in_radians()) * speed if abs(self.velocity[0]) <= self.MAX_SPEED else self.MAX_SPEED * dir_x
        self.velocity[1] = self.velocity[1] + math.sin(self._angle_in_radians()) * speed if abs(self.velocity[1]) <= self.MAX_SPEED else self.MAX_SPEED * dir_y
    ...
    def calculate_new_position(self, point):
        x, y = point
        new_x = (x * math.cos(self._angle_in_radians())) - (y * math.sin(self._angle_in_radians()))
        new_y = (x * math.sin(self._angle_in_radians())) + (y * math.cos(self._angle_in_radians()))
        return new_x, new_y
